...
- **★起動し直さない（2026-09-09追加）★**: どのスクリプトも投稿前にランダム遅延を挟むため、Bashツールのバックグラウンド実行が10分あたりで「打ち切られた」ように見えることがある。**プロセスは生きているので起動し直さないこと**。1本目と2本目が両方とも投稿に進んで二重投稿になりうる。状態は各スクリプトのstdoutログ（`Posting jitter:` / `[post jitter]` の行に、あと何分待つかが出ている）と `tasklist` のpythonの有無で確認する。2026-09-09に危険情報タスクで実際に起動し直しが行われた（遅延14分24秒・結果的に二重投稿は起きなかったが偶然）。daily-spotが31分・44分・58分の遅延後も同じ起動方法で投稿に成功しているのが、プロセスが生きている証拠
