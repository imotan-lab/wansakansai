...
- **★起動し直さない（2026-09-09追加）★**: どのスクリプトも投稿前にランダム遅延を挟むため、Bashツールのバックグラウンド実行が10分あたりで「打ち切られた」ように見えることがある。**プロセスは生きているので起動し直さないこと**。1本目と2本目が両方とも投稿に進んで二重投稿になりうる。状態は各スクリプトのstdoutログ（`Posting jitter:` / `[post jitter]` の行に、あと何分待つかが出ている）と `tasklist` のpythonの有無で確認する。2026-09-09に危険情報タスクで実際に起動し直しが行われた（遅延14分24秒・結果的に二重投稿は起きなかったが偶然）。プロセスが生きている根拠は2つ。①2026-09-09に13分20秒のスリープをバックグラウンドで走らせ、13分21秒後に完走することを直接実測した ②daily-spotが31分・44分・58分の遅延後も同じ起動方法で投稿に成功している
...
1. **今日のおすすめ** - `scripts/post_daily_spot.py`
   ...
   - 選出ロジック（2026-09-25改定・運営者の依頼「需要のありそうなところを中心に、でも同じ場所ばかりにしない」）:
     点数 = 間隔の係数 × (1 + 需要 + 写真3.0 + 訪問済み0.5 + 旬の季節タグ3.0 + ドッグラン0.3 + 入場無料0.2) × 府県の散らし、の重み付きランダム
     - 需要: Search Console のページ別表示回数（直近90日）を `2.0 × log10(1 + 表示/50)`、上限4.0。元データは `Documents/wansakansai/spot_demand.json`（ローカルのみ・週次SEOタスクがSTEP 1の7で毎週更新・`scripts/update_spot_demand.py`）
     - 写真: 写真があるとXのリンクカードにその写真が出る（無いとロゴ）
     - 間隔（掛け算）: 30日以内は出さない／30〜59日×0.25／60〜89日×0.5／90日以上×0.75／未投稿×1
     - 府県の散らし: 直前の投稿（おすすめ・新スポット）と同じ府県は×0.6
     - `stay-only` は除外。季節タグだけで季節外のスポットは、表示300回以上か写真ありの時だけ出す（文面は季節外なら季節の言葉を入れない）
     - 90日の試算（8回平均）: 表示300回以上か写真ありが約7割、表示50回未満が約2.5割、違うスポット約87/90、同じスポットは最多2回。`python scripts/post_daily_spot.py --simulate 90` で確かめられる（投稿も履歴の書き換えもしない）
...
4. **週次まとめ** - `scripts/post_weekly_digest.py` … **2026-09-08に廃止（稼働していない）**
   - 廃止理由: GA4でX経由の流入が28日間で24セッション（全体の0.7%）・平均滞在3秒と実測され、続ける根拠が無かった
   - スクリプト・タスク定義のアーカイブと復活手順: `Claude_backup/自動タスク/_廃止アーカイブ_weekly-digest_2026-09-08/`
   - 廃止前の仕様: 毎週土曜20:00 JST・タスクID `wansakansai-weekly-digest`・`git log --diff-filter=A --since="7 days ago"` で新規追加ファイルを検出＋90日以上未投稿スポットのピックアップ
