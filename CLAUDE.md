...
- **AdSense審査落ち** → adsbygoogle.js スクリプトと ad-slot div を全HTMLから一旦除去（2026-05-25実施）
  ...
  - **再申請ステータス: まだ再申請していない**。薄いページは解消済み。残る弱点は写真の少なさで、**2026-09-01時点で写真ありは263件中13件**。ユーザー方針＝**写真を入れてから再申請**
- **アフィリエイト収益化**: スポット詳細ページ最下部に「愛犬と泊まれる宿を探す」（楽天トラベル＋じゃらん/A8.net）を設置（実装は `js/spot.js` のみ・県別ディープリンク・rel="sponsored"・GAクリック計測あり）
  - 検証済みで修正不要。**A8リンクはサイト非依存（アカウント共通）**＝うちどころ/わんさかんさいで同一a8matでよい
...
- **★verifyタスクの修正はCodexが独立検証してから確定する（2026-09-01導入・STEP 8.5）★**　これまで間違い探しがClaude1体だけで、同じ見落としを2回繰り返しても気づけなかったため、別のAIに独立して確認させ2つの結論を突き合わせる
