...
- **★verifyタスクの修正はCodexが独立検証してから確定する（2026-09-01導入・STEP 8.5）★**　これまで間違い探しがClaude1体だけで、同じ見落としを2回繰り返しても気づけなかったため、別のAIに独立して確認させ2つの結論を突き合わせる
  - **修正が1件も無い日はCodexを呼ばない**。呼ぶのは書き込む直前の1回だけで、**1件ずつ呼ばない**（1施設10分以上かかる実測があり、5件を個別に回すとタスクが破綻する）
  - 判定の扱い: 一致→確定 ／ **不一致→その修正を取り消してメール通知**（人間が判断） ／ 確認できず→確定しログとメールに記録
  - **★Codexが落ちてもタスクを止めないこと★** 失敗・タイムアウト・未インストールの場合は修正を従来どおり確定し、`[codex-skip]` を記録してメールに「Codex検証は実行できず」と明記する。検証できないことを理由に更新を止めると、過去にあった「タスクが3日間サイレントに停止」を自分で作り出すことになるため
  - **Codexの出力は tail や head で切らない**（前半が失われて判定が読めなくなる。2026-09-01に実際に発生）。出力には検索ログと `ERROR codex_models_manager` の行が大量に混ざり（無害）、**最終回答が2回繰り返される**ことがある。判定は末尾を読む
  - **手動テストで無人実行の条件を確認済み（2026-09-01）**: 2項目のレビューで123秒・終了コード0・**承認待ちは発生しなかった**。5件分では長くなるためBashツールのtimeoutは420000ミリ秒に設定してある
  - テストではCodexが**現地看板しか根拠が無かった項目についても公式Webページを自力で見つけて裏を取った**（和歌山県公式観光サイト・白浜町公式）。Claudeが写真だけで満足して公式を探しに行かなかった見落としを拾えている
