...
- 末尾は必ず「。」で終わる
- **分量は必ず200字以上**（薄い解説を作らない。150字未満は不可）。施設の性格・規模・犬連れ条件（リード/エリア分離/サイズ制限/ワクチン等）・駐車場・トイレ・料金・営業時間・注意点を盛り込んで厚みを出す。過去に薄い解説92件を200字以上へ一括拡充済み（150字未満ゼロを達成）。**スポット追加・調査/再検証エージェントへの指示にも必ずこの200字以上基準を明記すること**（指示し忘れると薄い解説が混入する）
