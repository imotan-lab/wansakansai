...
★2026-09-04の実測。この2つの文言を消さないこと★
  ①「不一致は違う内容を実際に確認できた時だけ。見つからなかったは確認できず」
  ②「expires は予報終了日＋2〜4日。終了日そのものを答えてほしい」
同じ danger-017 に対して、
  修正前（09-04朝の本番）… 種別=一致 / 期限=不一致 / 要約=不一致 / 事実関係=不一致
  修正後（同日昼の再実行）… 4項目すべて一致
中身もデータも変えていない。変えたのはこのプロンプトだけ。
不一致3件はいずれも「Codexが根拠ページに行き当たらなかっただけ」で、
そのたびにClaudeが調べ直して時間を使っていた
（例: 日本気象協会8月31日の記事を jwa.or.jp だけ見て2日続けて見落とした）。

ただし1件1回の結果であって、常にこうなる保証ではない。
不一致が減ったからといって、判定をそのまま信用してよいことにはならない。
...
ALLOWED_TYPES = [
    "毒餌", "毒餌の疑い", "感染症", "危険な生き物", "有毒なもの", "その他の注意",
]
...
- 種別（type）は次の6つだけを使います。これ以外は使いません（気象は2026-09-20に載せない方針にした）。
{types}
...
1. **種別（type）** … 中身に合っていますか。上の6語のどれが正しいですか。
